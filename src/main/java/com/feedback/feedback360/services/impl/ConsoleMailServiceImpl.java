...
public class ConsoleMailServiceImpl implements MailService {
    ...
    public boolean sendFeedbackInvite(User user, ModuleFormation module) {
        log.info("""
            ...
            =================================""",
            user.getEmail(), module.getTitle(), user.getFirstName(), module.getId());
        return true;
    }

    @Override
    public boolean sendReminder(User user, ModuleFormation module, String template) {
        log.info("""
            ===== [MOCK EMAIL — REMINDER] =====
            To:   {}
            Body: {}
            ===================================""",
            user.getEmail(), template.replace("{moduleTitle}", module.getTitle()));
        return true;
    }
}